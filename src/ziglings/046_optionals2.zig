...
const Elephant = struct {
    ...
    tail: ?*Elephant = null, // Tail needs the Elephant ptr to be nullable
    visited: bool = false,
};
...
fn visitElephants(first_elephant: *Elephant) void {
    ...
    while (!e.visited) {
        ...
        // Place break statement on elephants tail pointing to null
        if (e.tail == null) break;

        // HINT: We want something similar to what `.?` does,
        // but instead of ending the program, we want to exit the loop...

        // the current elephant becomes the elephants tail, if it has one..
        e = e.tail.?;
    }
}
